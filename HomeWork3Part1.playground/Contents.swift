...
for index in 0..<cart.count {
    print("------------------- \(index+1) -------------------------------");
    print("Назва товару: \(cart[index].0), Ціна: \(cart[index].1) \(cart[index].2)");
    print("Сокет: \(cart[index].3), Процессор: \(cart[index].4)");
    print("------------------------------------------------------\n");
}
...
for (index, element) in cart.enumerated() {
    print("------------------- \(index+1) -------------------------------");
    print("Назва товару: \(element.0)");
    print("Процессор: \(element.4)");
    print("------------------------------------------------------\n");
}
...
var whileIndex = 0;
while(cart.count > whileIndex) {
    print("------------------- \(whileIndex+1) -------------------------------");
    print("Назва товару: \(cart[whileIndex].0)");
    print("Ціна: \(cart[whileIndex].1) \(cart[whileIndex].2)");
    print("------------------------------------------------------\n");
    whileIndex+=1;
}
...
var repeateWhileIndex = 0;
repeat {
    print("------------------- \(repeateWhileIndex+1) -------------------------------");
    print("Назва товару: \(cart[repeateWhileIndex].0)");
    print("Сокет: \(cart[repeateWhileIndex].3)");
    print("------------------------------------------------------\n");
    repeateWhileIndex+=1;
} while(cart.count > repeateWhileIndex)
...
func printFilteredProducts() {
    for (index, product) in cart.enumerated() {
        let (productName, price, currency, socket, _) = product
        if price < 5000.00 && socket == "s1151" {
            print("------------------- \(index + 1) -------------------------------")
            print("Назва товару: \(productName), Ціна: \(price) \(currency)")
            print("Сокет: \(socket)")
            print("------------------------------------------------------\n")
        }
    }
...
func printProductsByProcessor() {
    for (index, product) in cart.enumerated() {
        let (productName, price, currency, _, processor) = product
...
func printIntelProducts() {
    print("------------------- Intel -------------------------------")
    for (index, product) in cart.enumerated() {
        let (productName, _, _, _, processor) = product
        
        // Перевіряємо, чи процесор є "Intel"
        if processor == "Intel" {
...
func printProductsBelowMaxPrice(maxPrice: Double) {
    print("------------------- Товари з ціною менше \(maxPrice) -------------------------------")
    for (index, product) in cart.enumerated() {
        let (productName, price, currency, _, _) = product
        
        // Перевіряємо, чи ціна товару менше вказаної максимальної ціни
        if price < maxPrice {
            print("\(index + 1) Назва товару: \(productName), Ціна: \(price) \(currency)")
        }
    }
    print("------------------------------------------------------")
}

printProductsBelowMaxPrice(maxPrice: 4500.00)
...
func findMostExpensiveProductByProcessor(processor: String) -> ProductInfo? {
    ...
    for product in cart {
        let (_, price, _, _, productProcessor) = product
        
        if productProcessor == processor {
            if price > maxPrice {
                maxPrice = price
...
let mostExpensiveProduct = findMostExpensiveProductByProcessor(processor: "Intel");

if (mostExpensiveProduct != nil) {
    let (productName, price, currency, _, _) = mostExpensiveProduct!
    print("------------------- Найдорожчий товар за процесором Intel -------------------------------")
    print("Назва товару: \(productName), Ціна: \(price) \(currency)")
    print("-----------------------------------------------------------------------------------------\n")
} else {
    print("В кошику немає товарів з процесором Intel \n")
}
...
var myMotherBoard: MotherBoard = MotherBoard(socet: "s1151", processor: .intel)

print("Початкова материнська плата: socket = \(myMotherBoard.socet), processor = \(myMotherBoard.processor.rawValue)");


myMotherBoard.processor = .amd;

print("Змінена материнська плата: socket = \(myMotherBoard.socet), processor = \(myMotherBoard.processor.rawValue) \n");
...
class Product {
    ...
    init(name: String = "", price: Double = 0.0, currency: Currency = .usd, motherBoard: MotherBoard = MotherBoard(socet: "s1151", processor: .intel)) {
        self.name = name
        self.price = price
...
var myProduct = Product(name: "Example Product", price: 999.99, currency: .usd, motherBoard: MotherBoard(socet: "sAM4", processor: .amd))

print("Початковий продукт: name = \(myProduct.name), price = \(myProduct.price), currency = \(myProduct.currency.rawValue), motherboard socket = \(myProduct.motherBoard.socet), motherboard processor = \(myProduct.motherBoard.processor.rawValue)")

myProduct.name = "New Product Name"
myProduct.price = 1499.99
myProduct.currency = .eur
myProduct.motherBoard = MotherBoard(socet: "s1200", processor: .intel)

print("Змінений продукт: name = \(myProduct.name), price = \(myProduct.price), currency = \(myProduct.currency.rawValue), motherboard socket = \(myProduct.motherBoard.socet), motherboard processor = \(myProduct.motherBoard.processor.rawValue)")
